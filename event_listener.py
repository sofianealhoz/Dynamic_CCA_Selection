...
def trigger_analysis(dst_ip_str):
    ...
    if benchmark_type == 's' :
        ...
        try:
            ...
            print(f"--- ✅ Prediction for: {dst_ip_str} terminated ---")
            subprocess.run(
            ["python3", "calculate_averages.py", f"f_{filename}", "data/benchmark_data_troughput_and_srtt.csv", algo, env, col, benchmark_type],
            check=True, timeout=10
            )
    ...
    #f_ --> checker dans vm: les f_auto / f_... voir si pas pb de nom
        ...
        try:
            ...
            ["python3", "calculate_averages.py", filename, "data/benchmark_data_troughput_and_srtt.csv", algo, env, col, benchmark_type],
            check=True, timeout=10
            )
